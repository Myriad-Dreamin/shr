...
let versionToUploadInput = process.argv[2];

if (!versionToUploadInput) {
    console.error("Please provide the version to upload");
    process.exit(1);
}

if (versionToUploadInput.startsWith('v')) {
    versionToUploadInput = versionToUploadInput.slice(1);
}
const versionToUpload = versionToUploadInput;
...
const main = async () => {
    ...
    const fullChangelogLine = /\*\*Full Changelog\*\*: https:\/\/github.com\/Myriad-Dreamin\/shr\/compare\/v(\d+\.\d+\.\d+)\.\.\.v(\d+\.\d+\.\d+)/;
    ...
    const changelogPlain = changelogPlainRaw.replace(fullChangelogLine, (_match, p1, p2) => {
        ...
        return `\*\*Full Changelog\*\*: https://github.com/Myriad-Dreamin/shr/compare/v${p1}...v${versionToUpload}`;
    });
    if (!anyMatched) {
